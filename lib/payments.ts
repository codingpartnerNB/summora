...
async function createOrUpdateUser({sql, email, fullName, customerId, priceId, status}: {sql: any; email: string; fullName: string; customerId: string; priceId: string; status: string;}){
    try{
...
export async function handleCustomerSubscriptionDeleted({subscriptionId, stripe}: {subscriptionId: string; stripe: Stripe}){
